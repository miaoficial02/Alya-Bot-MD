const handler = async (m, { conn, isAdmin, isBotAdmin, quoted, mentionedJid }) => {
    ...
    if (!target) return m.reply('❌ Mencioná o respondé a un usuario para promoverlo.');

    try {
        await conn.groupParticipantsUpdate(m.chat, [target], 'promote');
        m.reply(`✅ @${target.split('@')[0]} ahora es admin.`, null, { mentions: [target] });
    } catch (err) {
        m.reply(`❌ Error al promover: ${err.message}`);
    }
};
